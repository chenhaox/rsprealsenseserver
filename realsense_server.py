...
import realsensecontroller as rsc


class EtherSenseServer(chenserver.AsyncServer):
    ...
    async def handle_read(self, reader, data = None):
        # self.realsensecontroller.frame_data
        data = await reader.read(1)
        if len(data) < 1:
            return None
        print("New data is ", data)
        command = int(data)
        if command not in servercode:
